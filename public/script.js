document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchTotalRainfallToday() {
        try {
            const response = await fetch('http://localhost:3000/api/rainfall/today');
    ...
    async function saveRainfallToDB(value) {
        ...
        try {
            await fetch('http://localhost:3000/api/rainfall', {
                method: 'POST',
                headers: {
    ...
    async function fetchHistoricalData(timeRange = 'daily') {
        try {
            const response = await fetch(`http://localhost:3000/api/historical-rainfall?range=${timeRange}`);
            if (!response.ok) {
